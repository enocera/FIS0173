...
class Model_ErrorEnsemble:
    ...
    def process_training_data(self, scaling='standardise', **kwargs):
        ...
        **kwargs can take on:
        ...
        # let sklearn handle the shuffling of the input data
        # the default is random so we will get a different split with every instance
    ...
    def baseline_model_errorensemble(self, layers, lr=0.001, activation='tanh', loss='mean_squared_error'):
        'define and compile model with fixing weight initialisers and a random dataset'
        # create model
        # make sure the parameter initialisation is fixed by setting the random seed in the glorot initialiser explicitly

        model = Sequential()
        model.add(Dense(layers[0], input_dim=(self.input_size), kernel_initializer = glorot_uniform(seed=1)))
        if activation == 'tanh':
        ...
        for i in range(1,len(layers)):
            model.add(Dense(layers[i], kernel_initializer = glorot_uniform(seed=i+1)))
            if activation == 'tanh':
                model.add(Activation(activations.tanh))
            elif activation == 'relu':
                model.add(Activation(activations.relu))
    ...
    def fit(
        ...
        self.model = self.baseline_model_errorensemble(layers=layers, lr=lr, activation=activation, loss=loss)
